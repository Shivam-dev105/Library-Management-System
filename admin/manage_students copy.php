...
<body>
                                    ...
                                    <ul class="pagination justify-content-center mb-0">
                                        ...
                                        <?php for ($p = 1; $p <= $total_pages; $p++): ?>
                                            <li class="page-item <?= ($page == $p) ? 'active' : ''; ?>">
                                                <a class="page-link ajax-page-link" href="#" data-page="<?= $p; ?>"><?= $p; ?></a>
                                            </li>
                                        <?php endfor; ?>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            const sessionFilter = document.querySelector("select[name='session_id']");
            const departmentFilter = document.querySelector("select[name='department_id']");
            const tableBody = document.getElementById("studentTableBody");
            const paginationContainer = document.getElementById("paginationContainer");
            let timeout = null;

            // Fetch function ab page number accept karega
            function fetchFilteredData(page = 1) {
                ...
                timeout = setTimeout(() => {
                    // URL mein query, filters aur page number bhej rahe hain
                    const url = `ajax/search_students_ajax.php?query=${encodeURIComponent(query)}&session_id=${encodeURIComponent(sessionId)}&department_id=${encodeURIComponent(departmentId)}&page=${page}`;

                    fetch(url)
                        .then(response => response.json()) // Ab hum response ko JSON me le rahe hain
                        .then(data => {
                            tableBody.innerHTML = data.table_html; // Table rows update
                            paginationContainer.innerHTML = data.pagination_html; // Pagination update
                        })
                        .catch(error => console.error("Error fetching data:", error));
                }, 300);
            }

            // Naya search/filter lagne par hamesha Page 1 se load karein
            searchInput.addEventListener("keyup", () => fetchFilteredData(1));
            sessionFilter.addEventListener("change", () => fetchFilteredData(1));
            departmentFilter.addEventListener("change", () => fetchFilteredData(1));

            // Pagination links par click ko handle karna (Event Delegation)
            paginationContainer.addEventListener("click", function(e) {
                e.preventDefault();
                // Check karein ki click "ajax-page-link" class wale anchor par hua hai
                if (e.target.classList.contains("ajax-page-link")) {
                    ...
                    if (!e.target.parentElement.classList.contains("disabled")) {
                        let selectedPage = e.target.getAttribute("data-page");
                        if (selectedPage) {
                            fetchFilteredData(parseInt(selectedPage));
                        }
                    }
                }
            });

            // Initial Load par data lana (agar already filters set hain)
            if (searchInput.value.trim() !== "" || sessionFilter.value !== "" || departmentFilter.value !== "") {
                fetchFilteredData();
            }
        });
    </script>
