...
if (isset($_POST['login'])) {
    ...
    /* ================= ADMIN LOGIN ================= */
    $adminQuery = mysqli_query($con, "SELECT * FROM admin WHERE email='$email'");
    if (mysqli_num_rows($adminQuery) > 0) {
        $adminData = mysqli_fetch_assoc($adminQuery);

        // ✅ password_verify for admin
        if (password_verify($password, $adminData['password'])) {

            if ($adminData['status'] == 1) {
                $_SESSION['admin'] = $email;
                // ✅ Update last login time
                mysqli_query($con, "UPDATE admin SET last_login = NOW() WHERE email = '".$adminData['email']."'");
                // echo "<script>alert('✅ Admin Login Successful');window.location.href='admin/dashboard.php'</script>";
                echo "<script>window.location.href='admin/dashboard.php'</script>";
                exit;
            } else {
                echo "<script>alert('⛔ Admin account inactive. Contact super admin');window.location.href='index.php'</script>";
                exit;
            }
        } else {
                ...
                <div class="row justify-content-center">
                    <div class="col-md-6 col-lg-5">
                        <div class="text-center mb-4">
                            ...
                            <h2 class="fw-bold" id="title">Admin Login</h2>
                        ...
                        <div class="card login-card p-4 p-md-5 bg-white custom-border">
                            ...
                            <form action="#" method="POST" id="login">
                                ...
                                <button type="submit" name="login" class="login btn btn-primary w-100 mb-3 shadow">Login</button>

                                <div class="text-center">
                                    <span class="text-muted small">Only Admin can login here</span>
